...
# NEW: Upstream-tracked path: llava/eval/model_vqa.py
...
def eval_model(args):
    ...
    if (args.use_random_depth or args.use_zero_depth) and not use_discrete_depth_tokens:
        print("[WARNING] Random/zero depth ablations requested but model does not use discrete depth tokens; options will be ignored.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-random-depth",
        action="store_true",
        help="Use random discrete depth tokens (ablation mode for discrete depth models)"
    )
    parser.add_argument(
        "--use-zero-depth",
        action="store_true",
        help="Use all zero discrete depth tokens (ablation mode for discrete depth models)"
    )
