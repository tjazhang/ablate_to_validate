...
# NEW: Upstream-tracked path: qwen-vl-finetune/qwenvl/train/trainer.py
...
# Use local model classes instead of transformers versions
from qwenvl.modeling_qwen2_5_vl import (
...
# Define ALL_LAYERNORM_LAYERS locally (removed from transformers.trainer in newer versions)
...
def replace_qwen2_vl_attention_class():
    ...
    # Use local model classes
    from qwenvl import modeling_qwen2_5_vl

    # Patch transformers qwen2_vl (for backward compatibility)
    ...
    # Patch local qwen2_5_vl model (the one we're actually using)
    modeling_qwen2_5_vl._flash_attention_forward = _flash_attention_forward
    modeling_qwen2_5_vl.Qwen2_5_VLModel._update_causal_mask = _update_causal_mask
...
# Custom Trainer to handle depth loss logging
class QwenVLTrainer(Trainer):
    """Custom trainer that logs depth loss separately."""
    
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        Compute loss and extract depth_loss for logging.
        ...
        # Extract depth_loss if available for logging
        if hasattr(outputs, "depth_loss") and outputs.depth_loss is not None:
            # Store depth_loss for logging (it will be automatically logged by Trainer)
            self.depth_loss = outputs.depth_loss.detach().item()
        
        return (loss, outputs) if return_outputs else loss
    
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:
        """
        Log metrics including depth_loss.
        
        Args:
            logs: Dictionary of metrics to log
            start_time: Optional start time for the training step
        """
        # Add depth_loss to logs if available
        if hasattr(self, 'depth_loss') and self.depth_loss is not None:
            logs['depth_loss_mse'] = self.depth_loss
            self.depth_loss = None
        
        # Call parent log method with both arguments if start_time is provided
