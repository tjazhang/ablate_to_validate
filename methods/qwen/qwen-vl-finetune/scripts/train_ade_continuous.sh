...
# NEW: Aurora path: qwen-vl-finetune/scripts/train_ade_continuous.sh
...
# Comma-separated to keep the example launch simple while still covering the
# main public variants: SigLIP, CLIP, and DINOv2.
DEPTH_ENCODER_NAMES="${DEPTH_ENCODER_NAMES:-google/siglip2-large-patch16-256,openai/clip-vit-large-patch14-336,facebook/dinov2-base}"

DEPTH_HEAD_TYPE="${DEPTH_HEAD_TYPE:-linear}"
DEPTH_PROJECTOR_TYPE="${DEPTH_PROJECTOR_TYPE:-linear}"
DEPTH_NORMALIZE="${DEPTH_NORMALIZE:-False}"
DEPTH_APPLY_SOFTMAX="${DEPTH_APPLY_SOFTMAX:-False}"
DEPTH_TEMPERATURE="${DEPTH_TEMPERATURE:-1.0}"
...
IFS=',' read -r -a DEPTH_ENCODER_LIST <<< "${DEPTH_ENCODER_NAMES}"

for depth_encoder_name in "${DEPTH_ENCODER_LIST[@]}"; do
    encoder_slug="$(echo "${depth_encoder_name}" | sed 's|/|_|g' | sed 's|-|_|g')"
    run_name="${RUN_NAME_PREFIX:-qwen2.5vl-3b-ade-continuous}-${NUM_EPOCHS}ep-${encoder_slug}-${DEPTH_HEAD_TYPE}_${DEPTH_PROJECTOR_TYPE}"
    ...
    ARGS=(
        ...
        --depth_apply_softmax "${DEPTH_APPLY_SOFTMAX}"
