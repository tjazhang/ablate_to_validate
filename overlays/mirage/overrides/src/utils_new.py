...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="vsp-spatial-reasoning", choices=["vsp-spatial-reasoning", "vsp-spatial-planning", "blink-jigsaw", "sat", "depth-reasoning"])
    ...
    # Ablation arguments (similar to model_vqa.py)
    parser.add_argument(
        "--use-random-latent",
        action="store_true",
        help="Use random latent embeddings (ablation mode for latent-based models)"
    )
    parser.add_argument(
        "--use-zero-latent",
        action="store_true",
        help="Use all zero latent embeddings (ablation mode for latent-based models)"
    )
    parser.add_argument(
        "--use-gt-latent",
        action="store_true",
        help="Use ground truth latent embeddings with fixed count from config (same as training)"
    )
    parser.add_argument(
        "--use-gt-latent-count-matched",
        action="store_true",
        help="Two-pass: generate to get count N, then use GT embeddings compressed to N tokens"
    )
    parser.add_argument(
        "--use-model-latent",
        action="store_true",
        help="Use model-generated latent embeddings (sanity check - should match baseline)"
    )
    parser.add_argument(
        "--use-random-latent-gt-dist",
        action="store_true",
        help="Use random latent embeddings with GT distribution (from helper image, ablation mode)"
    )
    parser.add_argument(
        "--use-random-latent-model-dist",
        action="store_true",
        help="Use random latent embeddings with model's own generation distribution (ablation mode)"
    )
    parser.add_argument(
