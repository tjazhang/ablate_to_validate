...
# Initialize wandb
wandb.init(
    project=os.environ.get("WANDB_PROJECT", "mirage-training"),
    name=f"{args.task}_{args.stage}_{args.epochs}ep",
    config={
        "model": args.model,
        "epochs": args.epochs,
        "task": args.task,
...
if args.stage in ['stage1']: 
    ...
    grad_checkpointing = True # Enable gradient checkpointing to reduce memory usage

config.compress_strategy = args.compress_strategy
config.latent_size = args.latent_size
config.stage = args.stage

# Clear GPU cache before loading model to prevent memory corruption
logging.info("Clearing GPU cache before model loading...")
...
if args.stage in ['stage1']:
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, config=config, device_map="auto", torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", cache_dir=cache_dir)
elif args.stage in ['stage2']:
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, config=config, device_map="auto", torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")

logging.info("Model loaded successfully.")
torch.cuda.empty_cache()
if torch.cuda.is_available():
    torch.cuda.synchronize()
logging.info("GPU synchronized after model loading.")
...
training_args = SFTConfig(
    ...
    run_name=f"{args.task}_{args.stage}_{args.epochs}ep",
)

# Clear GPU cache and synchronize before trainer initialization to prevent CUDA errors
logging.info("Clearing GPU cache before trainer initialization...")
torch.cuda.empty_cache()
torch.cuda.synchronize()
...
# Check if there are any checkpoints in the output directory to resume from
checkpoint_path = None
if os.path.exists(args.save_model_path):
    checkpoints = [d for d in os.listdir(args.save_model_path) if d.startswith("checkpoint-")]
    if checkpoints:
...
del model
torch.cuda.empty_cache()
logging.info("Training complete. GPU memory cleared.")
