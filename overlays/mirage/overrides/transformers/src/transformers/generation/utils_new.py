...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        # Custom generation kwargs that should be allowed (ablation flags, etc.)
        allowed_custom_kwargs = {"use_random_latent", "use_zero_latent", "use_gt_latent", "use_gt_latent_count_matched", "use_model_latent", "use_first_latent_repeat", "use_random_latent_gt_dist", "use_random_latent_model_dist", "gt_latent_mean", "gt_latent_std", "model_latent_mean", "model_latent_std", "gt_target_count"}
        
        for key, value in model_kwargs.items():
            if value is not None and key not in model_args and key not in allowed_custom_kwargs:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        first_repeat_latent_vec = None

        # Compute logging statistics in fp32 to avoid bf16 quantization artifacts.
        def _stats_fp32(t: torch.Tensor):
            t = t.float()
        ...
        while self._has_unfinished_sequences(
            ...
            # Extract custom ablation flags before passing to model (they're not model parameters)
            # These will be used later when processing latent embeddings
            model_inputs.pop("use_random_latent", None)
            model_inputs.pop("use_zero_latent", None)
            model_inputs.pop("use_gt_latent", None)
            model_inputs.pop("use_gt_latent_count_matched", None)
            model_inputs.pop("use_model_latent", None)
            model_inputs.pop("use_first_latent_repeat", None)
            model_inputs.pop("use_random_latent_gt_dist", None)
            model_inputs.pop("use_random_latent_model_dist", None)
            model_inputs.pop("gt_latent_mean", None)
            model_inputs.pop("gt_latent_std", None)
            model_inputs.pop("model_latent_mean", None)
            model_inputs.pop("model_latent_std", None)
            model_inputs.pop("gt_target_count", None)
            ...
            for i, token_str in enumerate(next_token_strs):
                if token_str == latent_start_idx:
                    in_latent_mode = True
                    latent_start = True
                    use_random_latent = model_kwargs.get("use_random_latent", False)
                    use_zero_latent = model_kwargs.get("use_zero_latent", False)
                    use_gt_latent = model_kwargs.get("use_gt_latent", False)
                    use_gt_latent_count_matched = model_kwargs.get("use_gt_latent_count_matched", False)
                    use_model_latent = model_kwargs.get("use_model_latent", False)
                    use_first_latent_repeat = model_kwargs.get("use_first_latent_repeat", False)
                    use_random_latent_gt_dist = model_kwargs.get("use_random_latent_gt_dist", False)
                    use_random_latent_model_dist = model_kwargs.get("use_random_latent_model_dist", False)
                    first_repeat_latent_vec = None
                    if use_random_latent or use_zero_latent or use_gt_latent or use_gt_latent_count_matched or use_model_latent or use_first_latent_repeat or use_random_latent_gt_dist or use_random_latent_model_dist:
                        if use_gt_latent:
                            ablation_type = "gt_fixed"
                        elif use_gt_latent_count_matched:
                            ablation_type = "gt_count_matched"
                        elif use_model_latent:
                            ablation_type = "model"
                            ...
                            ablation_type = "random_model_dist"
                        else:
                            ablation_type = "random" if use_random_latent else "zero"
                        logger.info(f"[Latent Mode] Entering latent mode with {ablation_type} latent ablation/injection enabled")
                    ...
                    if latent_end_num == cur_bs:    
                        ...
                        # Log count matching info for GT modes
                        if use_gt_latent and hasattr(self, "gt_latent_features"):
                            expected_count = self.model.config.latent_size if hasattr(self.model.config, 'latent_size') else 4
                            logger.info(f"[Latent Mode] Exiting latent mode after {latent_num} latent tokens")
                            logger.info(f"[GT Mode 1] Used fixed count: {expected_count} (from config)")
                        elif use_gt_latent_count_matched and hasattr(self, "gt_full_token_count"):
                            target_count = model_kwargs.get("gt_target_count", latent_num)
                            logger.info(f"[Latent Mode] Exiting latent mode after {latent_num} latent tokens")
                            logger.info(f"[GT Mode 2] Generated: {latent_num}, Target: {target_count}, GT Full: {self.gt_full_token_count}")
                            logger.info(f"[GT Mode 2] Compression ratio: {self.gt_full_token_count}/{latent_num} = {self.gt_full_token_count/max(latent_num, 1):.2f}x")
                        elif use_random_latent or use_zero_latent or use_model_latent or use_first_latent_repeat or use_random_latent_gt_dist or use_random_latent_model_dist:
                            logger.info(f"[Latent Mode] Exiting latent mode after {latent_num} latent tokens")
            ...
            if in_latent_mode and not latent_start:
                ...
                # Mode 1: GT with fixed count from config
                if use_gt_latent:
                    if latent_num == 1:
                        pixel_values_latent = model_kwargs.get("pixel_values_latent")
                        image_grid_thw_latent = model_kwargs.get("image_grid_thw_latent")
                        if pixel_values_latent is not None:
                            ...
                            batch_size = input_ids.shape[0]
                            hidden_size = latent_image_embeds.shape[-1]
                            
                            # Compress to FIXED latent_size (same as training)
                            if self.model.config.compress_strategy == "average":
                                latent_image_embeds = latent_image_embeds.view(batch_size, -1, hidden_size)
                                latent_size = self.model.config.latent_size
                                res = latent_image_embeds.shape[1] % latent_size
                                if res > 0: latent_image_embeds = latent_image_embeds[:, :-res, :]
                                group_size = latent_image_embeds.shape[1] // latent_size
                            ...
                            self.gt_latent_features = latent_image_embeds
                            logger.info(f"[GT Mode 1] Compressed GT to fixed count: {latent_image_embeds.shape[1]} tokens")
                        else:
                            logger.warning("[Latent Mode] GT latent requested but pixel_values_latent is missing!")
                            self.gt_latent_features = None

                    if hasattr(self, "gt_latent_features") and self.gt_latent_features is not None:
                        # Use sequentially from pre-compressed GT embeddings
                        if latent_num <= self.gt_latent_features.shape[1]:
                            cur_latent_embeds = self.gt_latent_features[:, latent_num-1:latent_num, :]
                        else:
                            # Model generating more than expected, cycle through
                            gt_idx = (latent_num - 1) % self.gt_latent_features.shape[1]
                            cur_latent_embeds = self.gt_latent_features[:, gt_idx:gt_idx+1, :]
                        
                        # Log distribution comparison on first latent token
                        if latent_num == 1:
                            gt_mean, gt_std = _stats_fp32(self.gt_latent_features)
                            model_output_mean, model_output_std = _stats_fp32(outputs.hidden_states)
                            
                            # Store statistics for aggregation
                            if not hasattr(self, '_ablation_stats'):
                                self._ablation_stats = {'model_output_means': [], 'model_output_stds': []}
                            self._ablation_stats['model_output_means'].append(model_output_mean)
                            self._ablation_stats['model_output_stds'].append(model_output_std)
                            
                            print(f"[Ablation] Using GT latent embeddings (Mode 1: fixed count)")
                            print(f"[Ablation] GT (reasoning image) - USED - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            print(f"[Ablation] Model Original Output       - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            print(f"[Ablation] Distribution Difference     - Mean: {abs(gt_mean - model_output_mean):.6f}, Std: {abs(gt_std - model_output_std):.6f}")
                            logger.info(f"[Ablation] GT Mode 1 (fixed) - USED - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                    else:
                        cur_latent_embeds = outputs.hidden_states
                
                # Mode 2: GT with count-matched compression
                elif use_gt_latent_count_matched:
                    if latent_num == 1:
                        pixel_values_latent = model_kwargs.get("pixel_values_latent")
                        image_grid_thw_latent = model_kwargs.get("image_grid_thw_latent")
                        gt_target_count = model_kwargs.get("gt_target_count")  # From first pass
                        
                        if pixel_values_latent is not None and gt_target_count is not None:
                            pixel_values_latent = pixel_values_latent.type(self.visual.dtype)
                            latent_image_embeds = self.visual(pixel_values_latent, grid_thw=image_grid_thw_latent)
                            ...
                            # Store full-resolution GT embeddings
                            self.gt_latent_features_full = latent_image_embeds.view(batch_size, -1, hidden_size)
                            self.gt_full_token_count = self.gt_latent_features_full.shape[1]
                            self.gt_target_count = gt_target_count
                            
                            # Compress GT to target count (from first pass) using averaging
                            if self.gt_full_token_count >= gt_target_count:
                                group_size = self.gt_full_token_count // gt_target_count
                                res = self.gt_full_token_count % gt_target_count
                                
                                # Average groups of GT embeddings
                                compressed_embeds = []
                                for i in range(gt_target_count):
                                    start_idx = i * group_size
                                    end_idx = (i + 1) * group_size
                                    if i < res:  # Distribute remainder
                                        start_idx += i
                                        end_idx += i + 1
                                    else:
                                        start_idx += res
                                    ...
                                    end_idx = min(end_idx, self.gt_full_token_count)
                                    if end_idx > start_idx:
                                        compressed_embeds.append(
                                            self.gt_latent_features_full[:, start_idx:end_idx, :].mean(dim=1, keepdim=True)
                                        )
                                
                                self.gt_latent_features_compressed = torch.cat(compressed_embeds, dim=1)
                            else:
                                # Rare case: need to upsample (repeat with interpolation)
                                self.gt_latent_features_compressed = self.gt_latent_features_full
                            
                            logger.info(f"[GT Mode 2] Compressed GT: {self.gt_full_token_count} → {self.gt_latent_features_compressed.shape[1]} tokens (target: {gt_target_count})")
                        else:
                            logger.warning("[Latent Mode] GT count-matched requested but pixel_values_latent or gt_target_count is missing!")
                            self.gt_latent_features_compressed = None

                    if hasattr(self, "gt_latent_features_compressed") and self.gt_latent_features_compressed is not None:
                        # Use sequentially from compressed GT embeddings
                        if latent_num <= self.gt_latent_features_compressed.shape[1]:
                            cur_latent_embeds = self.gt_latent_features_compressed[:, latent_num-1:latent_num, :]
                        else:
                            # Force latent end when GT is used up
                            logger.info(f"[GT Mode 2] GT embeddings exhausted at latent_num={latent_num} (available: {self.gt_latent_features_compressed.shape[1]}), forcing latent_end")
                            latent_end = True
                            in_latent_mode = False
                            next_tokens[:, None] = latent_end_idx
                            cur_latent_embeds = self.gt_latent_features_compressed[:, -1:, :]  # Use last token as placeholder
                        
                        # Log distribution comparison on first latent token
                        if latent_num == 1:
                            gt_mean, gt_std = _stats_fp32(self.gt_latent_features_compressed)
                            model_output_mean, model_output_std = _stats_fp32(outputs.hidden_states)
                            ...
                            print(f"[Ablation] Using GT latent embeddings (Mode 2: count-matched to {self.gt_target_count})")
                            print(f"[Ablation] GT (compressed) - USED      - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            print(f"[Ablation] Model Original Output       - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            print(f"[Ablation] Distribution Difference     - Mean: {abs(gt_mean - model_output_mean):.6f}, Std: {abs(gt_std - model_output_std):.6f}")
                            logger.info(f"[Ablation] GT Mode 2 (count-matched to {self.gt_target_count}) - USED - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                    else:
                        cur_latent_embeds = outputs.hidden_states
                
                elif use_model_latent:
                    # Use the model's own output (sanity check - should match baseline)
                    cur_latent_embeds = outputs.hidden_states
                    if latent_num == 1:
                        model_output_mean, model_output_std = _stats_fp32(outputs.hidden_states)
                        logger.info(f"[Ablation] Using model's own latent output (sanity check mode)")
                        logger.info(f"[Ablation] Model Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                else:
                    cur_latent_embeds = outputs.hidden_states
                
                # Apply ablation if requested (note: use_model_latent doesn't modify the embeddings)
                # Only log statistics on first latent token (latent_num == 1) to avoid spam
                if use_first_latent_repeat:
                    model_output_mean, model_output_std = _stats_fp32(cur_latent_embeds)
                    if first_repeat_latent_vec is None:
                        first_repeat_latent_vec = cur_latent_embeds.detach().clone()
                        if latent_num == 1:
                            if not hasattr(self, '_ablation_stats'):
                                self._ablation_stats = {'model_output_means': [], 'model_output_stds': []}
                            self._ablation_stats['model_output_means'].append(model_output_mean)
                            self._ablation_stats['model_output_stds'].append(model_output_std)
                            logger.info("[Ablation] Captured first model latent embedding for first-repeat mode")
                            logger.info(f"[Ablation] First latent stats - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                    else:
                        cur_latent_embeds = first_repeat_latent_vec.to(
                            device=cur_latent_embeds.device,
                            dtype=cur_latent_embeds.dtype,
                        )
                        if latent_num == 2:
                            logger.info("[Ablation] Repeating first model latent embedding for remaining latent tokens")
                elif use_random_latent:
                    # Pure random N(0,1) - standard normal distribution
                    if latent_num == 1:
                        # Calculate model's original output for comparison
                        model_output_mean, model_output_std = _stats_fp32(cur_latent_embeds)
                        ...
                        # Log both GT and model's original output for comparison
                        if hasattr(self, "gt_latent_features") and self.gt_latent_features is not None:
                            gt_mean, gt_std = _stats_fp32(self.gt_latent_features)
                            logger.info(f"[Ablation] Replacing model output with pure random N(0,1)")
                            logger.info(f"[Ablation] GT (reasoning image) - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            logger.info(f"[Ablation] Random N(0,1) (used) - Mean: 0.000000, Std: 1.000000")
                        else:
                            logger.info(f"[Ablation] Replacing model output with pure random N(0,1)")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            logger.info(f"[Ablation] Random N(0,1) (used) - Mean: 0.000000, Std: 1.000000")
                    
                    # Generate pure random values with N(0,1)
                    cur_latent_embeds = torch.randn_like(cur_latent_embeds)
                elif use_random_latent_gt_dist:
                    # Use GT distribution statistics (from helper image visual encoding)
                    gt_latent_mean = model_kwargs.get("gt_latent_mean")
                    gt_latent_std = model_kwargs.get("gt_latent_std")
                    
                    if gt_latent_mean is not None and gt_latent_std is not None:
                        if latent_num == 1:
                            # Get model's original output for comparison
                            model_output_mean, model_output_std = _stats_fp32(cur_latent_embeds)
                            ...
                            # Replace with random embeddings using GT distribution
                            logger.info(f"[Ablation] Replacing model output with random values using GT distribution")
                            logger.info(f"[Ablation] GT distribution (target) - Mean: {gt_latent_mean:.6f}, Std: {gt_latent_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                        
                        # Generate random values with GT mean and std
                        cur_latent_embeds = torch.randn_like(cur_latent_embeds) * gt_latent_std + gt_latent_mean
                    else:
                        logger.warning("[Ablation] GT distribution requested but statistics not provided; using zero embeddings as fallback")
                        cur_latent_embeds = torch.zeros_like(cur_latent_embeds)
                elif use_random_latent_model_dist:
                    # Use model's dynamic distribution (computed from current output)
                    model_output_mean, model_output_std = _stats_fp32(cur_latent_embeds)
                    ...
                    if latent_num == 1:
                        ...
                        if hasattr(self, "gt_latent_features") and self.gt_latent_features is not None:
                            gt_mean, gt_std = _stats_fp32(self.gt_latent_features)
                            logger.info(f"[Ablation] Replacing model output with random values (using model's dynamic distribution)")
                            logger.info(f"[Ablation] GT (reasoning image) - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            logger.info(f"[Ablation] Distribution Difference - Mean: {abs(gt_mean - model_output_mean):.6f}, Std: {abs(gt_std - model_output_std):.6f}")
                        else:
                            logger.info(f"[Ablation] Replacing model output with random values (using model's dynamic distribution)")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                    
                    # Generate random values with the same mean and std as model's current output
                    cur_latent_embeds = torch.randn_like(cur_latent_embeds) * model_output_std + model_output_mean
                elif use_zero_latent:
                    if latent_num == 1:
                        # Log GT and model's original output for comparison
                        model_output_mean, model_output_std = _stats_fp32(cur_latent_embeds)
                        ...
                        if hasattr(self, "gt_latent_features") and self.gt_latent_features is not None:
                            gt_mean, gt_std = _stats_fp32(self.gt_latent_features)
                            logger.info(f"[Ablation] Replacing model output with zeros")
                            logger.info(f"[Ablation] GT (reasoning image) - Mean: {gt_mean:.6f}, Std: {gt_std:.6f}")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                            logger.info(f"[Ablation] Zero embeddings (used) - Mean: 0.000000, Std: 0.000000")
                        else:
                            logger.info(f"[Ablation] Replacing model output with zeros")
                            logger.info(f"[Ablation] Model Original Output - Mean: {model_output_mean:.6f}, Std: {model_output_std:.6f}")
                    cur_latent_embeds = torch.zeros_like(cur_latent_embeds)
