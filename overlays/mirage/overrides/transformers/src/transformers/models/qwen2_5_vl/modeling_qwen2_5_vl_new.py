    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_latent is not None:
                ...
                # Only process pixel_values_latent if there are actually latent tokens in input_ids
                # During generation, subsequent forward passes won't have latent tokens
                if n_image_tokens > 0:
                    ...
                    # print(f"\n[DEBUG] GT Latent Processing:")
                    # print(f"  n_image_tokens (latent_pad count): {n_image_tokens}")
                    # print(f"  n_image_features (visual encoder output): {n_image_features}")
                    # print(f"  batch_size: {input_ids.shape[0]}")
                    # print(f"  latent_image_embeds.shape: {latent_image_embeds.shape}")
                    ...
                    if n_image_tokens != n_image_features:
                        ...
                        if self.model.config.compress_strategy == "average":
                            ...
                            # print(f"  Compression strategy: average")
                            # print(f"  n_image_tokens // batch_size: {n_image_tokens // batch_size}")
                            # print(f"  model.config.latent_size: {self.model.config.latent_size if hasattr(self.model.config, 'latent_size') else 'NOT SET'}")
                            # print(f"  latent_image_embeds after view: {latent_image_embeds.shape}")
        ...
        if self.config.stage == 'stage2' and not generate_mode:
            ...
            for latent_i in range(max_n_latents):
                ...
                if kv_cache == None:
                    ...
                    delta = (
                        (cache_position[0] + self.rope_deltas.to(inputs_embeds.device)).to(inputs_embeds.device)
                ...
                # Move hidden_states to the same device as inputs_embeds for multi-GPU support
